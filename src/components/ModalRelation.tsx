...
export default function ModalRelation({
  ...
  const radiusInsideBucket = (s: number, b: BucketKey, index: number, totalInBucket: number) => {
    ...
    const densitySpread = totalInBucket > 4 ? (Math.random() - 0.5) * (range * 0.25) : 0

    const finalRadius = base + indexVariation + jitter + hashVariation + densitySpread
        ...
        >
          ...
          >
            ...
            <motion.div
              ...
              style={{
                boxShadow: '0 0 80px rgba(99, 102, 241, 0.2), inset 0 0 60px rgba(255, 255, 255, 0.03)'
              }}
            >
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between py-2 px-3 bg-white/5 rounded-lg">
                          <span className="font-semibold text-sm text-gray-300">Similarity</span>
                          <span className="text-white font-bold">{(match.similarity * 100).toFixed(1)}%</span>
                        </div>
                        {match.confidence && (
                          <div className="flex items-center justify-between py-2 px-3 bg-white/5 rounded-lg">
                            <span className="font-semibold text-sm text-gray-300">Confidence</span>
                            <span className="text-white font-medium">{match.confidence}</span>
                          </div>
                        )}
                        {match.method && (
                          <div className="flex items-center justify-between py-2 px-3 bg-white/5 rounded-lg">
                            <span className="font-semibold text-sm text-gray-300">Method</span>
                            <span className="text-white font-medium">{match.method}</span>
                          </div>
                        )}
                      </div>
                    </motion.div>
          ...
          <div className="fixed bottom-8 right-8 bg-black/70 backdrop-blur-2xl rounded-2xl p-5 border border-white/20 pointer-events-auto z-[10001] shadow-2xl">
            <p className="text-white text-base font-bold mb-4">Similarity Scale</p>
