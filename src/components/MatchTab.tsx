...
export default function MatchesTab() {
  ...
  const [selectedArticle, setSelectedArticle] = useState<Article['article'] | null>(null)
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="p-8 rounded-2xl backdrop-blur-sm bg-white/5 border border-white/10">
          ...
          <p className="text-white/80 text-center text-sm">Loading article matches…</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        {selectedArticle ? (
          <div className="space-y-8">
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div className="min-w-0">
                  <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-lg bg-white/5 border border-white/10">
                    <span className="text-[11px] uppercase tracking-wide text-white/70">Selected Article</span>
                  </div>
                ...
                <div className="rounded-2xl border-2 border-white/15 bg-yellow-200/5 hover:bg-white/10 backdrop-blur-sm transition p-6 md:p-8">
                  <div className="flex flex-col items-center text-center gap-4">
                    <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-lg bg-yellow-500/5 border border-white/10">
                      <span className="text-[11px] uppercase tracking-wide text-white/70">Interactive</span>
                    </div>
                    ...
                    <div className="flex flex-wrap justify-center gap-2 pt-2">
                      ...
                      <span className="text-xs md:text-sm text-white/70 px-3 py-1 rounded-full border border-white/10 bg-white/5">
                        {((matches.matches_found / matches.total_osds_compared) * 100).toFixed(1)}% rate
                      </span>
                    </div>
                  </div>
                </div>
            ...
            {matches && (!matches.osd_matches || matches.osd_matches.length === 0) && (
              <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 text-center">
                <p className="text-white/80 font-medium">No OSD connections found for this article.</p>
                <p className="text-white/60 text-sm mt-1">Try selecting another article.</p>
              </div>
            )}
          </div>
        ) : (
          ...
          !showSelector && (
            <div className="flex items-center justify-center min-h-[60vh]">
              <div className="text-center p-10 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm max-w-md">
                <h3 className="text-xl font-semibold text-white mb-2">No Article Selected</h3>
