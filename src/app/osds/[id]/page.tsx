...
import {
  ...
  MapPin, ExternalLink, Beaker, BookOpen, FileText, FlaskConical, Users, Tag
} from 'lucide-react';
...
export default function OSDDetailPage() {
  ...
  const uniqueFactors = Array.from(new Set([
    ...(inv.factors || []),
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-1 py-3 border-b-2 whitespace-nowrap transition-colors text-sm sm:text-base ${
                      active ? 'border-white text-white' : 'border-transparent text-gray-400 hover:text-gray-300'
                    }`}
                  >
          ...
          <div className="rounded-2xl sm:rounded-3xl backdrop-blur-xl bg-white/10 border border-white/20 p-5 sm:p-6 shadow-2xl">
            {tab === 'overview' && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-lg sm:text-xl font-semibold text-white mb-3">Description</h2>
                ...
                {uniqueOrganisms.length > 0 && (
                  <div>
                    <h2 className="text-lg sm:text-xl font-semibold text-white mb-3">Organisms</h2>
                    <div className="flex flex-wrap gap-2">
                      {uniqueOrganisms.map((org, i) => (
                        <span key={i} className="px-3 py-1.5 rounded-lg bg-blue-500/30 backdrop-blur-sm border border-blue-400/30 text-blue-100 text-sm">
                          {org}
                        </span>
                      ))}
                    </div>
                ...
                {inv.publications && inv.publications.length > 0 && (
                  <div>
                    <h2 className="text-lg sm:text-xl font-semibold text-white mb-3">Publications</h2>
            ...
            {tab === 'samples' && (
              <div className="space-y-4">
                <p className="text-sm text-gray-300">
                  Total samples: <span className="font-bold text-white">{samples.length}</span>
                </p>
                <div className="overflow-x-auto">
                  <div className="space-y-3">
                    {samples.slice(0, 20).map((s, i) => (
                      <div key={i} className="p-4 rounded-xl bg-white/5 border border-white/10">
                        <h3 className="font-semibold text-white mb-2 text-sm sm:text-base">{s.sample_name}</h3>
                  ...
                  <div key={i} className="p-5 rounded-xl bg-white/5 border border-white/10">
                    ...
                    {a.platform && (
                      <p className="text-sm text-gray-300 mb-2">
                        <span className="text-gray-400">Platform:</span> {a.platform}
                      </p>
                    )}
                    {a.files && (
              ...
              <div className="space-y-6">
                {inv.mission && (
                  <div>
                    ...
                    <div className="p-4 rounded-xl bg-white/5 border border-white/10 space-y-2">
                      {inv.mission.name && <p className="text-white font-semibold">{inv.mission.name}</p>}
                      {inv.mission.start_date && (
                        <p className="text-sm text-gray-300">
                          {inv.mission.start_date} → {inv.mission.end_date || 'Ongoing'}
                        </p>
                      )}
                      {inv.mission.link && (
                        <a
                          ...
                          className="text-blue-300 hover:text-blue-200 text-sm inline-flex items-center gap-1"
                        >
                          View mission details <ExternalLink className="w-3 h-3" />
                        </a>
                      )}
                    </div>
                  </div>
                ...
                {inv.project && (
                  <div>
                    ...
                    <div className="p-4 rounded-xl bg-white/5 border border-white/10 space-y-2">
                      {inv.project.title && <p className="text-white font-semibold">{inv.project.title}</p>}
                      {inv.project.identifier && (
                        <p className="text-sm text-gray-400">ID: {inv.project.identifier}</p>
                      )}
                      {inv.project.type && (
                        <p className="text-sm text-gray-300">Type: {inv.project.type}</p>
                      )}
                      {inv.project.managing_center && (
                        <p className="text-sm text-gray-300">Center: {inv.project.managing_center}</p>
                      )}
                      {inv.project.funding && (
                        <p className="text-sm text-gray-300">Funding: {inv.project.funding}</p>
                      )}
                      {inv.project.link && (
                        <a
                    ...
                    <div className="space-y-2">
                      {inv.contacts.map((c: any, i: any) => (
                        <div key={i} className="p-3 rounded-xl bg-white/5 border border-white/10">
                          ...
                          {c.email && (
                            <a href={`mailto:${c.email}`} className="text-xs text-blue-300 hover:text-blue-200">
                              {c.email}
                            </a>
                          )}
                        </div>
